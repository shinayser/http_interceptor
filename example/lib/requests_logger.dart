...
class RequestsLogger extends InterceptorContract {
  ...
  @override
  Future<BaseRequest> interceptRequest({required BaseRequest baseRequest}) {
    if (request) {
      _printRequestHeader(options);
    }
    if (requestHeader) {
      final requestHeaders = <String, dynamic>{};
      requestHeaders.addAll(baseRequest.headers);
      requestHeaders['contentType'] = baseRequest.contentType;
      requestHeaders['followRedirects'] = baseRequest.followRedirects;
      _printMapAsTable(requestHeaders, header: 'Headers');
    }
    if (requestBody && baseRequest.method != 'GET') {
      final dynamic data = baseRequest.data;
      if (data != null) {
  ...
  void _printResponse(Response response) {
    if (response.data != null) {
      if (response.data is Map) {
        ...
        logPrint('║  HUGE ARRAY OF BYTES (SVG/LOTTIE)');
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          for (var i = 0; i < lines; ++i) {
            logPrint(
                '║${_indent(_tabs)} ${msg.substring(i * linWidth, math.min<int>(
                      i * linWidth + linWidth,
                      msg.length,
                    ))}');
          }
